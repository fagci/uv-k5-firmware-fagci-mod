...
void SETTINGS_SaveChannel(uint8_t Channel, uint8_t VFO, const VFO_Info_t *pVFO,
                          uint8_t Mode) {
    UART_LogSend("schn\r\n", 6);
    ...
    if (Mode == 2 || !IS_MR_CHANNEL(Channel)) {
        ...
        State8[3] =
            (pVFO->AM_CHANNEL_MODE << 4) | pVFO->FREQUENCY_DEVIATION_SETTING;
        State8[4] = 0 | (pVFO->BUSY_CHANNEL_LOCK << 4) |
                    (pVFO->OUTPUT_POWER << 2) | (pVFO->CHANNEL_BANDWIDTH << 1) |
                    (pVFO->FrequencyReverse << 0);
        State8[5] =
            (pVFO->DTMF_PTT_ID_TX_MODE << 1) | pVFO->DTMF_DECODING_ENABLE;
...
void SETTINGS_UpdateChannel(uint8_t Channel, const VFO_Info_t *pVFO,
                            bool bUpdate) {
    UART_LogSend("svalid\r\n", 8);
    ...
    if (bUpdate) {
        Attributes = 0 | (pVFO->SCANLIST1_PARTICIPATION << 7) |
                     (pVFO->SCANLIST2_PARTICIPATION << 6) | (pVFO->Band << 0);
