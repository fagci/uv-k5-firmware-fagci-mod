...
void RADIO_ConfigureChannel(uint8_t VFO, uint32_t Arg) {
    ...
    if (IS_VALID_CHANNEL(Channel)) {
        if (IS_MR_CHANNEL(Channel)) {
            Channel =
                RADIO_FindNextChannel(Channel, RADIO_CHANNEL_UP, false, VFO);
            if (Channel == 0xFF) {
    ...
    if (IS_MR_CHANNEL(Channel)) {
        ...
        EEPROM_ReadBuffer(0x0F50 + (Channel * 0x10),
                          gEeprom.VfoInfo[VFO].Name + 0, 8);
        EEPROM_ReadBuffer(0x0F58 + (Channel * 0x10),
                          gEeprom.VfoInfo[VFO].Name + 8, 2);
    }
    ...
    if (!gSetting_350EN) {
        ...
        if (pConfig->Frequency < 39999991) {
            pConfig->Frequency = 41001250;
        }
    }

    if (/*gEeprom.VfoInfo[VFO].Band == BAND2_108MHz && */ gEeprom.VfoInfo[VFO]
            .AM_CHANNEL_MODE) {
...
void RADIO_ConfigureSquelchAndOutputPower(VFO_Info_t *pInfo) {
    ...
    EEPROM_ReadBuffer(0x1ED0 + (Band * 0x10) + (pInfo->OUTPUT_POWER * 3), Txp,
                      3);
    pInfo->TXP_CalculatedSetting = FREQUENCY_CalculateOutputPower(
        Txp[0], Txp[1], Txp[2], LowerLimitFrequencyBandTable[Band],
...
void RADIO_SetupRegisters(bool bSwitchToFunction0) {
    ...
    BK4819_WriteRegister(BK4819_REG_7D,
                         gEeprom.MIC_SENSITIVITY_TUNING | 0xE94F);
    ...
    InterruptMask =
        0 | BK4819_REG_3F_SQUELCH_FOUND | BK4819_REG_3F_SQUELCH_LOST;
    ...
    if (!gRxVfo->IsAM) {
        ...
        switch (CodeType) {
            ...
            case CODE_TYPE_REVERSE_DIGITAL:
                ...
                InterruptMask =
                    0 | BK4819_REG_3F_CxCSS_TAIL | BK4819_REG_3F_CDCSS_FOUND |
            ...
            case CODE_TYPE_CONTINUOUS_TONE:
                ...
                InterruptMask =
                    0 | BK4819_REG_3F_CxCSS_TAIL | BK4819_REG_3F_CTCSS_FOUND |
                    BK4819_REG_3F_CTCSS_LOST | BK4819_REG_3F_SQUELCH_FOUND |
                    BK4819_REG_3F_SQUELCH_LOST;
                break;
            default:
...
void RADIO_SetTxParameters(void) {
    ...
    BK4819_SetupPowerAmplifier(gCurrentVfo->TXP_CalculatedSetting,
                               gCurrentVfo->pReverse->Frequency);
    SYSTEM_DelayMs(10);

    switch (gCurrentVfo->pReverse->CodeType) {
        case CODE_TYPE_CONTINUOUS_TONE:
            BK4819_SetCTCSSFrequency(
                CTCSS_Options[gCurrentVfo->pReverse->Code]);
            break;
        case CODE_TYPE_DIGITAL:
        case CODE_TYPE_REVERSE_DIGITAL:
            BK4819_SetCDCSSCodeWord(DCS_GetGolayCodeWord(
                gCurrentVfo->pReverse->CodeType, gCurrentVfo->pReverse->Code));
            break;
        default:
            BK4819_ExitSubAu();
            break;
