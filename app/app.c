...
void APP_CheckDTMFStuff(void) {
  ...
  if (8 < gDTMF_WriteIndex) {
    ...
    sprintf(String, "%s%c%s", gEeprom.ANI_DTMF_ID, gEeprom.DTMF_SEPARATE_CODE,
            gEeprom.KILL_CODE);
  ...
  if (1 < gDTMF_WriteIndex) {
    if (DTMF_CompareMessage(gDTMF_Received + gDTMF_WriteIndex - 2, "AB", 2,
                            true)) {
...
void FUN_000051e8(void) {
  ...
  if (g_CDCSS_Lost && gCDCSSCodeReceived == 0x01) {
    if (gCopyOfCodeType == CODE_TYPE_DIGITAL ||
...
void FUN_0000773c(void) {
  ...
  if (!IS_MR_CHANNEL(gInfoCHAN_A->CHANNEL_SAVE)) {
    RADIO_ApplyOffset(gInfoCHAN_A);
    RADIO_ConfigureSquelchAndOutputPower(gInfoCHAN_A);
    SETTINGS_SaveChannel(gInfoCHAN_A->CHANNEL_SAVE, gEeprom.RX_CHANNEL,
                         gInfoCHAN_A, 1);
    return;
  }
...
void FUN_000052f0(void) {
  ...
  if (g_20000377 == 0 && Value == 0 && gNextTimeslice40ms &&
...
void APP_SetFrequencyByStep(VFO_Info_t *pInfo, int8_t Step) {
  ...
  if (Frequency >= gUpperLimitFrequencyBandTable[pInfo->Band]) {
    ...
    pInfo->DCS[0].Frequency = FREQUENCY_FloorToStep(
...
void FUN_00007dd4(void) {
  ...
  Ch = RADIO_FindNextChannel(g_20000410 + gStepDirection, gStepDirection, true,
...
static int FM_ChecksChannelValid_and_FrequencyDeviation(uint16_t Frequency,
                                                        uint16_t LowerLimit) {
  uint16_t SNR;
  int16_t Deviation;
  uint16_t RSSI;
...
void APP_PlayFM(void) {
  if (!FM_ChecksChannelValid_and_FrequencyDeviation(gEeprom.FM_FrequencyToPlay,
                                                    gEeprom.FM_LowerLimit)) {
    if (gIs_A_Scan != 1) {
      gFmPlayCountdown = 0;
      g_20000427 = 1;
  ...
  while (BK4819_GetRegister(BK4819_REG_0C) & 1U) {
    ...
    if (Mask & BK4819_REG_02_VOX_LOST) {
      ...
      if (gEeprom.VOX_SWITCH) {
        ...
        if (gEeprom.DUAL_WATCH != DUAL_WATCH_OFF &&
            (gSystickFlag7 || (gSystickFlag7 == false && g_2000033A < 20))) {
          g_2000033A = 20;
          gSystickFlag7 = false;
        }
      }
...
static void FUN_00008334(void) {
  if (!gSetting_KILLED) {
    ...
    if (gCurrentFunction != FUNCTION_4 &&
        gCurrentFunction != FUNCTION_MONITOR && gStepDirection == 0 &&
        g_20000381 == 0 && !gFmRadioMode) {
      if (g_200003B4 == 1) {
        if (g_VOX_Lost) {
...
void APP_Update(void) {
  ...
  if (gScreenToDisplay != DISPLAY_SCANNER && gStepDirection && gSystickFlag9 &&
      !gPttIsPressed && gVoiceWriteIndex == 0) {
  ...
  if (gScreenToDisplay != DISPLAY_SCANNER &&
    ...
    if (gSystickFlag7 && gVoiceWriteIndex == 0) {
      if (gStepDirection == 0 && g_20000381 == 0) {
        if (!gPttIsPressed && !gFmRadioMode && g_200003BC == 0 &&
  ...
  if (gFM_Step && gScheduleFM && gCurrentFunction != FUNCTION_MONITOR &&
      gCurrentFunction != FUNCTION_4 && gCurrentFunction != FUNCTION_TRANSMIT) {
    APP_PlayFM();
    gScheduleFM = false;
  }
  ...
  if (gSchedulePowerSave) {
    if (gEeprom.BATTERY_SAVE == 0 || gStepDirection || g_20000381 ||
        gFmRadioMode || gPttIsPressed || gScreenToDisplay != DISPLAY_MAIN ||
        gKeyBeingHeld || g_200003BC) {
  ...
  if (gBatterySaveCountdownExpired && gCurrentFunction == FUNCTION_POWER_SAVE &&
    ...
    if (gThisCanEnable_BK4819_Rxon) {
      ...
      if (gEeprom.DUAL_WATCH != DUAL_WATCH_OFF && gStepDirection == 0 &&
          g_20000381 == 0) {
        FUN_00007f4c();
        g_20000382 = 0;
      }
      ...
      gBatterySave = 10;
      gThisCanEnable_BK4819_Rxon = false;
    } else if (gEeprom.DUAL_WATCH == DUAL_WATCH_OFF || gStepDirection ||
               g_20000381 || g_20000382) {
...
void APP_TimeSlice500ms(void) {
  ...
  if (g_2000037E == 1) {
    ...
    if ((gBatteryCurrent < 0x1f5) &&
        (gBatteryCurrentVoltage <= gBatteryCalibration[3])) {
      return;
    }
  ...
  if (gCurrentFunction != FUNCTION_TRANSMIT) {
    ...
    if ((gFM_Step == 0 || gAskToSave) && gStepDirection == 0 &&
        g_20000381 == 0) {
      ...
      if (gScreenToDisplay != DISPLAY_AIRCOPY &&
          (gScreenToDisplay != DISPLAY_SCANNER || (1 < gScanState))) {
        ...
        if (g_20000393 != 0) {
          g_20000393--;
          if (g_20000393 == 0) {
            if (gInputBoxIndex || g_200003BA == 1 ||
            ...
            gAskToSave = false;
            gAskToDelete = false;
            if (gFmRadioMode && gCurrentFunction != FUNCTION_4 &&
...
LAB_00004b08:
  ...
  if (g_200003BC != 0 && gCurrentFunction != FUNCTION_TRANSMIT &&
      gCurrentFunction != FUNCTION_4) {
...
void FUN_000075b0(void) {
  ...
  RADIO_InitInfo(gInfoCHAN_A, gInfoCHAN_A->CHANNEL_SAVE, gInfoCHAN_A->Band,
                 gInfoCHAN_A->pDCS_Current->Frequency);
...
static void APP_ProcessKey_MAIN(KEY_Code_t Key, bool bKeyPressed,
                                bool bKeyHeld) {
...
static void APP_ProcessKey_SCANNER(KEY_Code_t Key, bool bKeyPressed,
                                   bool bKeyHeld) {
  switch (Key) {
  case KEY_0:
  case KEY_1:
...
static void APP_ProcessKey_AIRCOPY(KEY_Code_t Key, bool bKeyPressed,
                                   bool bKeyHeld) {
  switch (Key) {
  case KEY_0:
  case KEY_1:
...
static void APP_ProcessKey(KEY_Code_t Key, bool bKeyPressed, bool bKeyHeld) {
  ...
  if (!bKeyPressed) {
    ...
    if (gFlagSaveChannel) {
      SETTINGS_SaveChannel(gTxRadioInfo->CHANNEL_SAVE, gEeprom.TX_CHANNEL,
                           gTxRadioInfo, gFlagSaveChannel);
  ...
  if ((gStepDirection && Key != KEY_PTT && Key != KEY_UP && Key != KEY_DOWN &&
       Key != KEY_EXIT && Key != KEY_STAR) ||
      (g_20000381 && Key != KEY_PTT && Key != KEY_UP && Key != KEY_DOWN &&
       Key != KEY_EXIT && Key != KEY_STAR && Key != KEY_MENU)) {
  ...
  if (gRequestSaveChannel) {
    if (!bKeyHeld) {
      SETTINGS_SaveChannel(gTxRadioInfo->CHANNEL_SAVE, gEeprom.TX_CHANNEL,
                           gTxRadioInfo, gRequestSaveChannel);
      if (gScreenToDisplay != DISPLAY_SCANNER) {
        g_2000039A = 1;
      }
    } else {
